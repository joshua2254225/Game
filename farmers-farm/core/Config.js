...
================================================================================
 ...
 USED BY     : src/core/GameEngine.js and src/core/GameLoop.js already
               (files 9 and 10, for DEBUG_MODE and MAX_DELTA_TIME).
               src/input/InputTranslator.js (file 17) for INPUT.*. Later:
...
export const CONFIG = deepFreeze({
  ...
  // ============================================================
  // INPUT — camera feel and joystick tuning.
  // Consumed by Phase 1's InputTranslator.js.
  // ============================================================

  INPUT: {
    /** PLACEHOLDER. How far (0-1, as a fraction of the joystick's full
     *  travel) the thumb must move off-center before a direction counts
     *  as pressed. Too low and a joystick that isn't perfectly re-centered
     *  on release causes phantom drift; too high and small deliberate
     *  nudges get ignored. Needs real hands-on tuning once
     *  VirtualJoystickDevice.js exists to actually test against. */
    JOYSTICK_DEADZONE: 0.2,

    /** PLACEHOLDER. Multiplies a raw horizontal drag distance (mouse or
     *  touch, in screen pixels) into the unitless amount CameraController.js
     *  will rotate by once it exists in Phase 3 — that file, not this
     *  number, decides what the unit actually means (degrees, radians, or
     *  something else entirely). Only meaningfully tunable by feel once
     *  there's an actual camera to drag. */
    CAMERA_ROTATE_SENSITIVITY: 0.005,

    /** PLACEHOLDER. Same idea as above, for wheel-scroll / pinch-distance
     *  input feeding CameraController.js's zoom. */
