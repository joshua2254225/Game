...
================================================================================
 ...
 PHASE       : Phase 0 — Skeleton (originally file 9; updated in Phase 1's
               final message to wire in InputManager.js)
 DEPENDS ON  : src/core/GameLoop.js, src/core/EventBus.js, src/core/Config.js,
               src/core/Constants.js, and now src/input/InputManager.js
               (file 23) — created in the constructor, started in start(),
               ticked LAST in update(), stopped in stop().
...
export class GameEngine {
  ...
  /** Stops the engine's game loop and input handling. Mirrors start(). */
  stop() {
    this.gameLoop.stop();
  ...
  /**
   * Runs once per frame, before render(). Mostly still a placeholder —
   * the one real piece is the LAST line.
   *
   ...
   */
  update(deltaTime) {
    // Future systems that READ input (Phase 3's CameraController, Phase 7's
    // PlayerController, Phase 8's VehicleController) will be called ABOVE
    // this line once they exist.

    // MUST be last: see InputManager.js's and CommandQueue.js's own headers
    // for why calling this before everything above has read this frame's
    // commands would make discrete/value commands disappear too early.
